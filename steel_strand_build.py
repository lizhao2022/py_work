...
def steel_d_2(sp1, sp2, sp3, bh):# 梁高2-2.3底板束坐标
	# 底板束形状设计参数
...
def steel_d_3(sp1, sp2, sp3, bh):#梁高2.5底板束坐标
	# 底板束形状设计参数
...
def steel_t_2(sp1, sp2, sp3):# 梁高2-2.3顶板束坐标
	# 顶板束形状设计参数
	xt_1=[10.5, 7.5]
...
def steel_t_3(sp1, sp2, sp3):# 梁高2.5顶板束坐标
	# 顶板束形状设计参数
	xt_1=[0.15, 2, 2.7]
	xt_2=[15.5, 12, 8.5, 5]
	xt_c=3
...
def steel_det_elenum(std, sx, nx):# 计算钢束起终点对应单元号
...
def steel_strand_build(span, end_seams, web, beam_height, node_x):
	...
	# 腹板束形状设计参数
	if beam_height >=2.5:
		xf_ends=[0.15, 0.15, 0.15, 0.15, 0.15]# 腹板束端部起终点水平位置
		...
		f_r=[[0]+[30]*10+[0]]*len(xf_ends)# 腹板束弯曲半径
		fra=[20, 20, 15, 10, 6]
		for i in range(5):
			f_r[i][3:5]=[fra[i]]*2
			f_r[i][7:9]=[fra[i]]*2
		...
		f_r=[[0]+[10]*10+[0]]*len(xf_ends)#　腹板束弯曲半径
	# 建立腹板束坐标
	f_x=[[]]*len(xf_ends)
	f_y=[[]]*len(xf_ends)
